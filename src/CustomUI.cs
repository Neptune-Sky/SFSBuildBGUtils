...
namespace SFSBuildGridUtils
{
    ...
    public static class CustomUI
    {
        public static NumberInput CreateNumberInput(Transform parent, int width, int height,
            ...
            int posY = 0)
        {
            int decimalsToKeep = charLimit - Math.Floor(defaultVal).ToString(CultureInfo.InvariantCulture).Length;
            if (Math.Floor(defaultVal) - defaultVal != 0) decimalsToKeep -= 1;
            var toReturn = new NumberInput
            {
                textInput = Builder.CreateTextInput(parent, width, height, posX, posY, defaultVal.Round(decimalsToKeep).ToString(CultureInfo.InvariantCulture), onChange),
